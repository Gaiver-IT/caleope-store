...
cat > "${_SECRETS}" <<ENV
UPTIME_KUMA_PORT_WEB=${UPTIME_KUMA_PORT_WEB}
ENV
chmod 600 "${_SECRETS}"
...
authentik_register_app() {
    ...
    if [ -z "${APP_UUID}" ]; then
        local PROV_RESP PROV_ID
        PROV_RESP=$(curl -sf --max-time 10 -X POST -H "${HA}" -H "${HJ}" "${BASE}/providers/proxy/" \
            -d "{\"name\":\"${APP_NAME} Proxy\",\"authorization_flow\":\"${FLOW_UUID}\",\"mode\":\"forward_single\",\"external_host\":\"${APP_URL}\"}") || return 1
        PROV_ID=$(echo "${PROV_RESP}" | python3 -c "import json,sys; print(json.load(sys.stdin)['pk'])") || return 1
...
if [ -d "${CALEOPE_BASE_DIR}/apps-installed/authentik" ]; then
    if authentik_register_app "Uptime Kuma" "uptime-kuma" "https://uptime-kuma.${CALEOPE_DOMAIN#*.}"; then
...
Uptime Kuma est démarré.
Interface : http://<IP>:${UPTIME_KUMA_PORT_WEB}

Première connexion : créer un compte admin directement dans l'interface.
INFO

echo "✓ Uptime Kuma prêt — interface : http://<IP>:${UPTIME_KUMA_PORT_WEB}"
