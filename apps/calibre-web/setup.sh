...
cat > "${_SECRETS}" <<ENV
CALIBREWEB_PORT_WEB=${CALIBREWEB_PORT_WEB}
ENV
chmod 600 "${_SECRETS}"
...
if [ -d "${CALEOPE_BASE_DIR}/apps-installed/authentik" ]; then
    if authentik_register_app "Calibre-Web" "calibre-web" "https://calibre.${CALEOPE_DOMAIN#*.}"; then
...
Calibre-Web est démarré.
Interface : http://<IP>:${CALIBREWEB_PORT_WEB}
...
echo "✓ Calibre-Web prêt — http://<IP>:${CALIBREWEB_PORT_WEB}"
