#!/bin/bash
# setup.sh — Forgejo (forge Git auto-hébergée, fork Gitea)
set -euo pipefail
echo "→ Préparation de Forgejo..."

CONFIG_DIR="${CALEOPE_BASE_DIR}/app-config/${CALEOPE_APP_ID}"
DATA_DIR="${CALEOPE_BASE_DIR}/app-data/forgejo"
...
# version passe par « caleope install forgejo --force », donc par ce script :
...
#     Error: failed to connect to `user=forgejo database=forgejo`
# Les données étaient intactes — l'app ne pouvait simplement plus les atteindre.
# Constaté en montant 12 → 15 : le témoin déposé en base a survécu, l'app non.
#
...
# ── bootstrap.sh (crée le compte admin via CLI forgejo) ─────────────────────────
# Tourne dans un container forgejo avec user=git et data monté.
# forgejo CLI a accès à app.ini et peut créer le premier admin sans auth.
cat > "${CONFIG_DIR}/bootstrap.sh" << 'BOOTSTRAP'
#!/bin/sh
MAX_WAIT=180
WAITED=0

echo "→ Forgejo bootstrap : attente de l'API HTTP (forgejo:3000)..."
until wget -qO- http://forgejo:3000/api/v1/version >/dev/null 2>&1; do
    sleep 5
    WAITED=$((WAITED + 5))
...
EXISTS=$(wget -qO- "http://forgejo:3000/api/v1/users/${GITEA_ADMIN_USER}" 2>/dev/null | grep -c '"login"' || true)
if [ "${EXISTS}" -gt 0 ]; then
...
if [ -d "${CALEOPE_BASE_DIR}/apps-installed/authentik" ]; then
    ...
    if [ -f "${AK_SECRETS}" ]; then
        ...
        if [ -n "${AK_TOKEN}" ] && [ -n "${AK_DOMAIN}" ]; then
            AK_PORT=$(grep "^CALEOPE_PORT_WEB=" "${CALEOPE_BASE_DIR}/apps-installed/authentik/app.env" 2>/dev/null | cut -d= -f2-) || true
            ...
            echo "  → Configuration OIDC Forgejo dans Authentik..."
...
" 2>/dev/null)
                ...
                if [ -n "${PROV_PK}" ]; then
                    curl -s --max-time 10 -X POST -H "${AK_HA}" -H "${AK_HJ}" \
                        ...
                        -d "{\"name\":\"Forgejo\",\"slug\":\"forgejo-sso\",\"provider\":${PROV_PK},\"meta_launch_url\":\"https://${CALEOPE_DOMAIN}/\"}" \
                        >/dev/null 2>&1 || true
...
' "${CALEOPE_APP_DIR}/compose.yml" > /tmp/gitea_compose_sso.yml && \
                    ...
                    docker exec forgejo forgejo admin auth add-oauth \
                        ...
                        --auto-discover-url "${AK_INTERNAL_URL}/application/o/forgejo-sso/.well-known/openid-configuration" \
                    ...
                    docker exec forgejo-db sh -c "
psql -U forgejo -d forgejo -c \"UPDATE login_source SET cfg = REPLACE(cfg::text, '${AK_INTERNAL_URL}', 'https://${AK_DOMAIN}')::json WHERE name = 'Authentik' AND is_active = true;\"
" 2>/dev/null || true

                    echo "  ✓ Forgejo OIDC configuré dans Authentik (PK=${PROV_PK})"
                else
                    echo "  ⚠ Erreur création provider OIDC Forgejo"
                fi
            else
...
cat > "${CALEOPE_APP_DIR}/post-install.txt" << EOF
  ...
  ┌──────────────────────────────────────────────────────────────────┐
  │                   Forgejo — Forge Git auto-hébergée                │
  ├──────────────────────────────────────────────────────────────────┤
...
echo ""
echo "  ╔══════════════════════════════════════════════════════╗"
echo "  ║              Forgejo — Identifiants admin              ║"
echo "  ╠══════════════════════════════════════════════════════╣"
