...
cat > "${_SECRETS}" <<ENV
FILEBROWSER_PORT_WEB=${FILEBROWSER_PORT_WEB}
ENV
chmod 600 "${_SECRETS}"
...
if [ -d "${CALEOPE_BASE_DIR}/apps-installed/authentik" ]; then
    if authentik_register_app "File Browser" "filebrowser" "https://filebrowser.${CALEOPE_DOMAIN#*.}"; then
...
File Browser est démarré.
Interface : http://<IP>:${FILEBROWSER_PORT_WEB}

Identifiants par défaut : admin / admin
Changer le mot de passe immédiatement après la première connexion.
INFO

echo "✓ File Browser prêt — http://<IP>:${FILEBROWSER_PORT_WEB}"
