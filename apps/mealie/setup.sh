...
cat > "${_SECRETS}" <<ENV
MEALIE_PORT_WEB=${MEALIE_PORT_WEB}
...
if [ -d "${CALEOPE_BASE_DIR}/apps-installed/authentik" ]; then
    if authentik_register_app "Mealie" "mealie" "https://mealie.${BASE_DOMAIN}"; then
...
Mealie est démarré.
Interface : http://<IP>:${MEALIE_PORT_WEB}

Identifiants par défaut : changeme@email.com / MyPassword
Changer le mot de passe admin immédiatement.
Inscriptions désactivées par défaut (ALLOW_SIGNUP=false).
INFO

echo "✓ Mealie prêt — http://<IP>:${MEALIE_PORT_WEB}"
