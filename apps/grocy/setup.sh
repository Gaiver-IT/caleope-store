...
if [ -d "${CALEOPE_BASE_DIR}/apps-installed/authentik" ]; then
    if authentik_register_app "Grocy" "grocy" "https://grocy.${CALEOPE_DOMAIN#*.}"; then
...
Grocy est démarré.
Interface : http://<IP>:${GROCY_PORT_WEB}

Identifiants par défaut : admin / admin
Changer le mot de passe immédiatement dans Administration > Utilisateurs.
INFO

echo "✓ Grocy prêt — http://<IP>:${GROCY_PORT_WEB}"
